...
&НаКлиенте
Процедура ПринтераПринтерПриИзменении(Элемент)
	// Вставить содержимое обработчика.	Стр =1;
	Стр = ТекущийЭлемент.ТекущиеДанные;
	Стр.Производитель = ПолучитьПроизводителя(Стр.Принтер);
КонецПроцедуры
...
Функция ПолучитьПроизводителя(Принтер) 	 
	Производитель = Принтер.Производитель;
	  Возврат Производитель; 
КонецФункции // ПолучитьПроизводителя()
     
&НаКлиенте
Процедура ПринтераПроизводительПриИзменении(Элемент)
